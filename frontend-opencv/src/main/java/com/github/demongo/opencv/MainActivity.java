...
import com.github.demongo.opencv.pipeline.FinalStep;
...
public class MainActivity extends AppCompatActivity implements CvCameraViewListener2 {
    ...
    public Mat onCameraFrame(CvCameraViewFrame frame) {
        ...
        Snapshot snap = new Snapshot(currentFrame, 1, 1);
        NoiseEstimationStep noiseEstimationStep = new NoiseEstimationStep(snap);
        BlurEstimationStep blurEstimationStep = new BlurEstimationStep();

        Log.e(TAG, "onCameraFrame: Start pipeline");
        Snapshot lastSnapshot = noiseEstimationStep.next(blurEstimationStep)
                .next(finalStep)
                .getSnapshot(); //only for debugging/drawing best frame

        if(lastSnapshot != null) {
            displayFrame(lastSnapshot.mat);
        }
